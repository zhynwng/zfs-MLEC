...
int dmu_buf_hold_array_by_bonus(dmu_buf_t *db_fake, uint64_t offset,
				  ...
				  uint64_t len, zio_priority_t pri)
{
	...
	if (len == 0)
	{ /* they're interested in the bonus buffer */
...
int dmu_free_long_range(objset_t *os, uint64_t object,
			  ...
			  void *buf, uint32_t flags)
{
	...
	if (dn->dn_maxblkid == 0)
	{
		uint64_t newsz = offset > dn->dn_datablksz ? 0 : MIN(size, dn->dn_datablksz - offset);
 ...
 * dmu_write().
 */
int dmu_assign_arcbuf_by_dnode(dnode_t *dn, uint64_t offset, arc_buf_t *buf,
							   dmu_tx_t *tx)
{
	dmu_buf_impl_t *db;
...
int dmu_assign_arcbuf_by_dbuf(dmu_buf_t *handle, uint64_t offset, arc_buf_t *buf,
							  dmu_tx_t *tx)
{
	int err;
...
int dmu_object_set_blocksize(objset_t *os, uint64_t object, uint64_t size, int ibs,
							 dmu_tx_t *tx)
{
	dnode_t *dn;
	int err;
...
void dmu_write_policy(objset_t *os, dnode_t *dn, int level, int wp, zio_prop_t *zp)
{
	...
	else
	{
		...
		checksum = (dedup_checksum == ZIO_CHECKSUM_OFF) ? zio_checksum_select(dn->dn_checksum, checksum) : dedup_checksum;
		 ...
		 * exclusive.
		 */
		nopwrite = (!dedup && (zio_checksum_table[checksum].ci_flags & ZCHECKSUM_FLAG_NOPWRITE) &&
	...
	zp->zp_zpl_smallblk = DMU_OT_IS_FILE(zp->zp_type) ? os->os_zpl_special_smallblock : 0;
...
restart:
	...
	{
		err = dnode_next_offset(dn, DNODE_FIND_HAVELOCK | (hole ? DNODE_FIND_HOLE : 0), off, 1, 1, 0);
	}
...
void __dmu_object_info_from_dnode(dnode_t *dn, dmu_object_info_t *doi)
{
	dnode_phys_t *dnp = dn->dn_phys;

	doi->doi_data_block_size = dn->dn_datablksz;
	doi->doi_metadata_block_size = dn->dn_indblkshift ? 1ULL << dn->dn_indblkshift : 0;
 ...
 */
void dmu_object_size_from_db(dmu_buf_t *db_fake, uint32_t *blksize,
...
void dmu_object_dnsize_from_db(dmu_buf_t *db_fake, int *dnsize)
{
	dmu_buf_impl_t *db = (dmu_buf_impl_t *)db_fake;
	dnode_t *dn;
